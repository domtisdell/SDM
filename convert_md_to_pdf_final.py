...
Uses mermaid-cli for reliable diagram rendering
"""
...
/* Special notice for failed diagrams */
.diagram-notice {
...
<div class="mermaid-diagram">
    <img src="data:image/png;base64,{img_base64}" alt="Diagram {diagram_counter}"/>
</div>
'''
        else:
            # Fallback - create a styled notice
            ...
            # Extract some key information from the diagram
            lines = mermaid_code.strip().split('\n')
            summary = f"This {diagram_type} shows the relationships and data flow in the WSA steel taxonomy."
            
            return f'''
<div class="mermaid-diagram">
    <div class="diagram-notice">
        <strong>Diagram {diagram_counter}: {diagram_type}</strong><br>
        {summary}<br>
        <em>Note: For the interactive diagram, please view the original markdown file with a mermaid-compatible viewer.</em>
    </div>
</div>
'''
    
    # Replace all mermaid blocks with rendered images or notices
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python convert_md_to_pdf_final.py <input_dir> <output_dir>")
        sys.exit(1)
    ...
    try:
        ...
        print("Using mermaid-cli for diagram rendering")
    except:
        print("Warning: mermaid-cli (mmdc) not found. Diagrams will show as notices.")
    ...
    for md_file in md_files:
        try:
            print(f"\nConverting: {os.path.relpath(md_file, input_dir)}")
